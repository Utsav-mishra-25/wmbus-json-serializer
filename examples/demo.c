...
static GatewayData create_sample_data(void) {
    ...
    safe_strcpy(data_points[0].timestamp, "2024-01-15 08:00", MAX_TIMESTAMP);
    safe_strcpy(data_points[0].meter_datetime, "2024-01-15 08:00", MAX_TIMESTAMP);
    data_points[0].total_value = 123.456f;
    safe_strcpy(data_points[0].status, "OK", MAX_STATUS);
    
    safe_strcpy(data_points[1].timestamp, "2024-01-15 08:15", MAX_TIMESTAMP);
    safe_strcpy(data_points[1].meter_datetime, "2024-01-15 08:15", MAX_TIMESTAMP);
    data_points[1].total_value = 123.789f;
    safe_strcpy(data_points[1].status, "OK", MAX_STATUS);
    
    safe_strcpy(data_points[2].timestamp, "2024-01-15 08:30", MAX_TIMESTAMP);
    safe_strcpy(data_points[2].meter_datetime, "2024-01-15 08:30", MAX_TIMESTAMP);
    data_points[2].total_value = 124.012f;
    ...
    safe_strcpy(readings[0].meter, "aquamaster", MAX_METER);
    safe_strcpy(readings[0].device_id, "wmbus_water_88765432", MAX_DEVICE_ID);
    ...
    safe_strcpy(readings[1].device_id, "wmbus_power_55123789", MAX_DEVICE_ID);
    safe_strcpy(readings[1].unit, "kWh", MAX_UNIT);
    readings[1].data = data_points;
    readings[1].data_count = 3;
    
    GatewayData gateway_data;
    safe_strcpy(gateway_data.gateway_id, "gateway_alpha_001", MAX_GATEWAY_ID);
    safe_strcpy(gateway_data.date, "2024-01-15", MAX_DATE);
    safe_strcpy(gateway_data.device_type, "stromleser", MAX_DEVICE_TYPE);
    gateway_data.interval_minutes = 15;
    gateway_data.total_readings = 4;
...
int main(void) {
    ...
    printf("Buffer usage: %d/%d bytes (%.1f%%)\n", 
           ...
           (bytes_written * 100.0) / MAX_BUFFER_SIZE);
